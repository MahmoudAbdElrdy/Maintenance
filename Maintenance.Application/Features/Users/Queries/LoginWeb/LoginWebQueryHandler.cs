...
{
    public class LoginWebQueryHandler : IRequestHandler<LoginWebQuery, ResponseDTO>
    {
        ...
        public async Task<ResponseDTO> Handle(LoginWebQuery request, CancellationToken cancellationToken)
        {
            try
            {

                var personalUser = await _userManager.Users.Where(x => x.IdentityNumber == request.NationalId).FirstOrDefaultAsync();
                ...
                var personalVefiy = await _userManager.Users.Where(x => x.IdentityNumber == request.NationalId && x.Code == request.Code).FirstOrDefaultAsync();
